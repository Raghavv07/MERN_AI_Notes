...
export const getLeaderboard = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { type = 'notes', limit = '10' } = req.query as { type?: string; limit?: string };
    const limitNum = Math.min(parseInt(limit) || 10, 50);
    ...
    if (type === 'streak') {
      // Leaderboard by study streak
      leaderboard = await User.find({ studyStreak: { $gt: 0 } })
        .sort({ studyStreak: -1, longestStreak: -1 })
        .limit(limitNum)
        .select('name studyStreak longestStreak totalNotesGenerated');

      // Get current user's rank
      const usersWithHigherStreak = await User.countDocuments({
        studyStreak: { $gt: user.studyStreak },
      });
      userRank = usersWithHigherStreak + 1;
    } else if (type === 'completed') {
      // Leaderboard by completed topics
      leaderboard = await User.find({ completedTopics: { $gt: 0 } })
        .sort({ completedTopics: -1 })
        .limit(limitNum)
        .select('name completedTopics totalNotesGenerated');

      const usersWithMoreCompleted = await User.countDocuments({
        completedTopics: { $gt: user.completedTopics },
      });
      userRank = usersWithMoreCompleted + 1;
    } else {
      // Default: Leaderboard by total notes generated
      leaderboard = await User.find({ totalNotesGenerated: { $gt: 0 } })
        .sort({ totalNotesGenerated: -1 })
        .limit(limitNum)
        .select('name totalNotesGenerated studyStreak completedTopics');

      const usersWithMoreNotes = await User.countDocuments({
        totalNotesGenerated: { $gt: user.totalNotesGenerated },
      });
      userRank = usersWithMoreNotes + 1;
    }
    ...
    const leaderboardWithRank = leaderboard.map((u, index) => ({
      rank: index + 1,
      name: u.name,
      totalNotesGenerated: u.totalNotesGenerated,
